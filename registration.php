...
if (isset($_POST['form1'])) {
    ...
    if ($valid == 1) {
        ...
        try {
            ...
            $statement1->execute(array($_POST['username'], $_POST['password'], 'user', false));

            $statement2 = $pdo->prepare("INSERT INTO tbl_account_details (account_id, customer_name, phone, email) VALUES (?,?,?,?)");
            $statement2->execute(array($pdo->lastInsertId(), $_POST['customer_name'], $token, $_POST['phone'], $_POST['email']));
            ...
            if (mail($to, $subject, $message, $headers)) {
                ...
                $pdo->rollBack();
                $error_message .= 'Không thể gửi email xác minh tài khoản<br>';
            }
